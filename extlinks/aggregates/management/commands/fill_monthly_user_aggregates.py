...
class Command(BaseCommand):
        ...
        if options["collections"]:
            ...
            for col_id in options["collections"]:
                ...
                daily_aggregation_exists = (
                    UserAggregate.objects.filter(filter_query).exclude(day=0).exists()
                )
    ...
    def _get_start_date_filter(self, collection_id=None):
        ...
        latest_date_aggregation = UserAggregate.objects.filter(base_query).aggregate(
            Max("full_date")
        )["full_date__max"]
    ...
    def _process_aggregation(self, collection_id=None, start_date_filter=Q()):
        ...
        daily_aggregations = (
            UserAggregate.objects.filter(daily_aggregations_query)
            .exclude(day=0)
            .order_by(
                "organisation_id",
                "collection_id",
        ...
        for aggregation in daily_aggregations:
            ...
            if prev_item is not None and (
                ...
                or prev_item.username != aggregation.username
                ...
                if (
                    ...
                    logger.info(
                        f"Monthly UserAggregate for organisation {prev_item.organisation_id} "
    ...
    def _save_aggregation(
        ...
        existing_monthly_aggregate = UserAggregate.objects.filter(
            ...
            username=aggregation_last_day.username,
